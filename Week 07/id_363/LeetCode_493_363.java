...
public class SolutionReversePair493 {
    ...
    public int reversePairs(int[] nums) {
        ...
        int cnt = doReversePairs(nums, 0, nums.length - 1);
        return cnt;
    }

    public int doReversePairs(int[] nums, int left, int right) {
        if (left >= right) {
            return 0;
        }
        int mid = (left + right) >> 1;
        int leftCnt = doReversePairs(nums, left, mid);
        int rightCnt = doReversePairs(nums, mid + 1, right);
        int intervalCnt = merge(nums, left, mid, right);
        return leftCnt + rightCnt + intervalCnt;
    }

    private int merge(int[] nums, int left, int mid, int right) {
        // 用来计数
        int cnt = 0, cj = mid + 1;
        for (int c = left; c <= mid; c ++) {
            while (cj <= right && (long)nums[c] > 2 * (long)nums[cj]) {
                cj ++;
            }
            // 记录left那边的数组 每个元素可以贡献的count
            cnt += cj - mid - 1;
        }
